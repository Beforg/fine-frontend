...
export class PerfilService {
  ...
  getClientesInfo(page: number): Observable<any> {
    return this.http.get<any>(`${this.apiUrl}/clientes?page=${page}&size=10&sort=nome,asc`, { headers: this.headers }).pipe(
       tap(response => {
         console.log("Informações dos clientes recebidas:", response);
       }),
       catchError(error => {
